...
function Network.CreateMessage(units,updates,moves,currentTeam)
    ...
    for i=1,#units,1 do--iPos,iHp
        ...
        str=str..tostring(units[i].Orientation)..","..tostring(units[i].Health)..","..units[i].Formation..","..units[i].OpenOrder..tostring(units[i].CurrentTarget.Name)..";"
        netmsg=netmsg..str
    end
...
function Network.DecodeMessage(message)
    ...
    local name, x, y, theta, hp, formation, order, currentTarget = unit:match("([^:]+):([^,]+),([^,]+),([^,]+),([^,]+),([^,]+),([^,]+),([^,]+)")
    ...
    for i=1,#units,1 do
        for i2=1,#team,1 do
            if team[i2].Name==units[i].Name then
                ...
                team[i2].OpCurrentTargetenOrder=units[i].CurrentTarget
                break
            end
        end
    end
